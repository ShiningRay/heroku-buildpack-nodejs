...
yarn_node_modules() {
  ...
  cd "$build_dir" || return
  monitor "yarn-install" yarn install --production="$production" --frozen-lockfile --ignore-engines --ignore-platform --prefer-offline 2>&1
}
...
yarn_prune_devdependencies() {
  ...
  if [ "$NODE_ENV" == "test" ]; then
    ...
    cd "$build_dir" || return
    monitor "yarn-prune" yarn install --frozen-lockfile --ignore-engines --ignore-platform --ignore-scripts --prefer-offline 2>&1
    meta_set "skipped-prune" "false"
  fi
}
